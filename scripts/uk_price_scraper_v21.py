#!/usr/bin/env python3
"""
UK Cigar Price Scraper v21
- JJ Fox: Use brand category pages instead of search
- Sautter: Back to Playwright with better stealth
- Fixed My Smoking Shop selectors

FIXES from v20:
1. JJ Fox uses /cigars/brand/{brand}-cigars.html pages
2. Sautter uses Playwright with stealth (HTTP requests got 403)
3. Better product matching for JJ Fox (pack size buttons)
"""
...
def load_jjfox_brand(brand):
    ...
    html = fetch_page(url, '.prod-hed', 3)
    if not html:
        _jjfox_brand_cache[brand_l] = []
        return []
    ...
    for prod_hed in soup.select('.prod-hed'):
        try:
            name_el = prod_hed.select_one('a')
            if not name_el:
                continue
            
            name = name_el.get_text(strip=True)
            
            # Get parent card to find prices
            card = prod_hed.find_parent('div', class_=lambda x: x and 'product' in x.lower()) or prod_hed.parent
            
            # Find the "From £X" price
            price_el = card.select_one('.from-price') if card else None
            if not price_el:
                price_el = prod_hed.find_next(string=re.compile(r'£[\d,]+'))
            ...
            # Try to find pack size options
            # JJ Fox shows buttons like "Single cigar", "Box of 25", "Box of 10"
            box_prices = {}
            
            # Look for size buttons in the card
            if card:
                for btn in card.select('button, .pack-option, [class*="pack"]'):
                    btn_text = btn.get_text(strip=True).lower()
                    
                    # Try to get price from data attribute or nearby element
                    btn_price = btn.get('data-price') or btn.get('data-amount')
                    if btn_price:
                        price = parse_price(btn_price)
                    else:
                        price = base_price  # Fall back to base price
                    
                    if 'single' in btn_text:
                        box_prices[1] = price
                    else:
                        box_match = re.search(r'box\s*(?:of\s*)?(\d+)', btn_text)
                        if box_match and price:
                            box_prices[int(box_match.group(1))] = price
            
            # If no box prices found, try to extract from name
            if not box_prices and base_price:
                box_size = extract_box_size(name)
                if box_size:
                    box_prices[box_size] = base_price
...
def find_jjfox(cigar):
    ...
    for p in products:
        if match_product(p['name'], cigar['brand'], cigar['name'], cigar['box_size']):
            # Check if we have a price for this box size
            if cigar['box_size'] in p.get('box_prices', {}):
                price = p['box_prices'][cigar['box_size']]
                if price and price > 30:
                    RETAILER_STATS['JJ Fox']['found'] += 1
                    return price, 'JJ Fox'
            # Fall back to base price if box size matches product name
            elif p.get('base_price') and extract_box_size(p['name']) == cigar['box_size']:
                RETAILER_STATS['JJ Fox']['found'] += 1
                return p['base_price'], 'JJ Fox'
...
def main():
    print("=" * 60)
    print("UK CIGAR PRICE SCRAPER v21")
    print("=" * 60)
    print(f"Date: {datetime.now()}")
    print("Fixes: JJ Fox brand pages, Sautter Playwright stealth")
