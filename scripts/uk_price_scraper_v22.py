#!/usr/bin/env python3
"""
UK Cigar Price Scraper v22
- JJ Fox: Fixed pack size extraction from buttons within li.product-item
- JJ Fox: Only return price if box size matches available pack sizes
- Removed 30% "lowest" rule - now flags box size discrepancies
- Better price aggregation logic
- Added new retailers: Cigar Club, Davidoff London
...
5. JJ Fox (jjfox.co.uk) - Brand pages with pack size extraction
...
def find_jjfox(cigar):
    ...
    for p in products:
        if match_product(p['name'], cigar['brand'], cigar['name'], cigar['box_size']):
            # Check if requested box size is in available packs
            if cigar['box_size'] in p.get('available_packs', []):
                # Check if we have a known price for this box size
                price = p.get('box_prices', {}).get(cigar['box_size'])
                if price and price > 30:
                    RETAILER_STATS['JJ Fox']['found'] += 1
                    return price, 'JJ Fox'
                # Box size available but price unknown (it's not the "From" price)
                # Don't return - we can't be sure of the price
...
def load_davidoff_brand(brand):
    ...
    html = fetch_page(url, '.product-card, .product-item', 4)
    if not html:
        _davidoff_brand_cache[brand_l] = []
        return []
    ...
    for item in soup.select('.product-card, .product-item, .grid-product'):
        try:
            name_el = item.select_one('.product-card__title, .product-item__title, h3, a')
            price_el = item.select_one('.price, .money, [data-product-price]')
            
            name = name_el.get_text(strip=True) if name_el else None
            
            # Extract price - Shopify often uses "from £X" format
            price = None
            if price_el:
            ...
            sold_out = item.select_one('.sold-out, .badge--sold-out, [class*="sold"]')
            if sold_out and 'sold' in sold_out.get_text(strip=True).lower():
                continue  # Skip sold out items
            
            if name and price and price > 10:  # Lower threshold as these might be singles
                products.append({
                    'name': name,
                    'price': price,
                    'box_size': extract_box_size(name)
